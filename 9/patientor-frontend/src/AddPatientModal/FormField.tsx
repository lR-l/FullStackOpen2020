...
export const TypeSelectField: React.FC<TypeSelectFieldProps> = ({ name, label, options, setType, setErrors }: TypeSelectFieldProps) => (
  <Form.Field>
    <label>{label}</label>
    <Field
      as="select"
...
export const NumberField: React.FC<NumberProps> = ({ field, label, min, max }) => {
  const checkNumberFieldValue = field.value === undefined ? "" : field.value;
  return (
    <Form.Field>
      <label>{label}</label>
      <Field {...field} type="number" min={min} max={max} value={checkNumberFieldValue} />
